...
const Drop = () => {
  ...
  const { status, teamsListRes, toolsAssignedRes, isMainLoading, pagiLoading, getInventoryCategoriesRes, teamMembersByIdRes, getToolsListRes, inventoryLocationsRes, membersOverallRes } = useSelector((state: any) => state.MainReducer);
  ...
  const handleTakeTool = () => {
    ...
    dispatch(dropToolRequest(payload));
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <HeaderContainer>
        <View style={styles.headerContent}>
          ...
          <Text style={styles.headerTitle}>Drop Tool</Text>
      ...
      >
        ...
        <DropdownInput label="Return To" value={selectedSource?.name || selectedSource?.title || 'Select'} onPress={() => openModal('Source')} />
        ...
        <DropdownInput label="Select Tool to Drop" value={selectedTools?.length > 0 ? "Select More Tools" : "Select"} onPress={() => openModal('Tool')} />
        ...
        <TouchableOpacity style={styles.confirmBtn} activeOpacity={0.8} onPress={handleTakeTool}>
          <Image source={ICONS.drop} style={styles.confirmBtnIcon} />
          <Text style={styles.confirmBtnText}>Confirm Drop</Text>
        </TouchableOpacity>
