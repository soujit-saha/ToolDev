...
import { getInventoryListRequest, getTeamMembersByIdRequest } from '../../redux/reducer/MainReducer';
import { goBack, navigate } from '../../utils/helper/RootNavigation';
import { useFocusEffect } from '@react-navigation/native';
import Loader from '../../utils/helper/Loader';

const TeamsDetails = ({ route }: any) => {
  const insets = useSafeAreaInsets();
  const dispatch = useDispatch();

  console.log('1234567', route?.params?.item)

  const { getInventoryListRes, isMainLoading, teamMembersByIdRes } = useSelector(
  ...
  useFocusEffect(
    React.useCallback(() => {
      const delayDebounceFn = setTimeout(() => {
        dispatch(getTeamMembersByIdRequest({ team_id: route?.params?.item?.id, search: searchQuery }));
      }, 500);
  ...
  const renderMemberCard = (
    ...
    id: string,
    status: string,
    tools: { tool_name: string; state: string; status: string }[] | null
  ) => {
    return (
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.logsList}>
          {teamMembersByIdRes && teamMembersByIdRes?.length > 0 ? (
            teamMembersByIdRes?.map((item: any, index: number) => (
              renderMemberCard(
                item?.name || 'Unknown',
